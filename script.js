...
async function clientSideRag(message, topK, temperature) {
    ...
    // 4. If we have a Groq API key, run generation in client browser via Groq
    if (apiKey && apiKey.trim() !== '') {
        console.log("Found Groq API key. Generating answer using client-side Groq API...");
        const systemPrompt = `You are the official AI assistant for Jodhpur Institute of Engineering & Technology (JIET).
...
Use markdown for formatting (bullet points, bold text). Keep it concise but comprehensive.`;
        ...
        for (const model of candidateModels) {
            try {
                ...
                if (!response.ok) {
                    ...
                    lastError = new Error(`Groq API Error: ${errMsg}`);
                    continue;
                }
                ...
                if (data.choices && data.choices[0] && data.choices[0].message) {
                    ...
                    lastError = new Error("Invalid response format from Groq API.");
                    continue;
                }
            } catch (err) {
                lastError = err;
        ...
        // No Groq API Key: return the top chunk content directly as fallback
        console.log("No Groq API key found. Falling back to direct Local RAG content match...");
        return {
            answer: topChunks[0],
            context_used: topChunks,
            is_genai: false
        ...
        try {
            ...
            if (statusBadge) {
                if (clientResult.is_genai) {
                    statusBadge.className = 'rag-status-badge active-mode';
                    statusBadge.querySelector('.status-text').textContent = 'Client RAG + Groq';
                } else {
                    statusBadge.className = 'rag-status-badge local-mode';
