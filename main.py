...
async def chat(request: ChatRequest):
    ...
    # Determine Groq API Key: check request first, then fallback to .env / environment variable
    effective_api_key = (request.api_key.strip() if request.api_key and request.api_key.strip() else None) or os.getenv("GROQ_API_KEY")
    
    # Generate response with Groq if API key is available
    if effective_api_key and effective_api_key.strip() != "":
        try:
            cleaned_key = re.sub(r'[^a-zA-Z0-9_-]', '', effective_api_key)
            client = Groq(api_key=cleaned_key)
            
            system_prompt = (
                "You are the official AI assistant for Jodhpur Institute of Engineering & Technology (JIET).\n"
                "You must answer the user's question politely and professionally, using ONLY the information provided in the Context below.\n"
                "If the context doesn't contain the answer, state clearly that you don't know based on the provided document.\n"
            ...
            def run_groq():
                candidate_models = ["qwen/qwen3.8-27b", "groq/compound-mini", "openai/gpt-oss-120b"]
                last_err = None
            ...
            answer = await asyncio.wait_for(
                loop.run_in_executor(executor, run_groq),
                timeout=12.0
            )
            is_genai_active = True
        except asyncio.TimeoutError:
            print("Groq API timed out, falling back to direct RAG.")
